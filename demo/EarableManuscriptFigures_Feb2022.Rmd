...
# setwd("~/Documents/Projects/EarableManuscript_2021") 
rm(list=ls())
...
library(dplyr);library(yingtools2)
library(data.table);library(ifultools);library(stringr);library(ggplot2)
library(Hmisc);library(tidyr);library(hues)
...
tmp_c<-samp
library(circlize);library(ComplexHeatmap);library(viridis);library(RColorBrewer)
...
super_col <- c("Time"= "#9857ff","Amplitude" = "#8f7536", "Frequency" = "gray",
               ...
               "Variance" = "#FF69B4","other" = "#f69ea0","MAG" = "black",
               "Phosphatidylethanolamine" = "orange","Phosphatidylinositol" = "purple",
               "Neutral Complex Lipids SUM" = "green","Partially Characterized Molecules" = "gray",
               "Amino Acid" = "#728cff", "Peptide" = "#3c53b9","Xenobiotics" = "hotpink",
               "Carbohydrate" = "#ff9135","Cofactors and Vitamins" = "#889977") 
...
mg0 <- read.table("./Earable.Dec2021.processed.data.all.tsv",sep = "\t",header = T) %>% data.frame()
mg0_0 <- read.table("./Earable.June2021.processed.data.all.tsv",sep = "\t",header = T) %>% data.frame()

mg0[,4:165] <- as.numeric(as.matrix(mg0[,4:165]))
mg0_0[,4:165] <- as.numeric(as.matrix(mg0_0[,4:165]))

mg0_0 <- reshape2::melt(mg0_0, id.vars = c("Person", "time", "Task.Label"), variable.name = "variables")
...
View(mg2[,c("Person","Task.Label","time","index1")])
